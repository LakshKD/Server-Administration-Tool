<html>
	<head>
		<title>Add New Network Interface</title>
	</head>
	<body>
		...
		<h1><b><u>ADD A NEW NETWORK INTERFACE</u></b></h1>
		<table border=2><tr><th><img src=interface.gif></th><tr></table><hr>
		<form action="" method="post">
		<table height=50% width=50% bgcolor=#cccccc>
		<tr height=5%>
		<th width=50% bgcolor=#9999ff>&nbsp;&nbsp;&nbsp;&nbsp;Add a new network interface</th></tr>
		<tr height=9%><td width=24%>
		&nbsp;&nbsp;&nbsp;&nbsp;Name</td><td width=2%> : </td><td width=24%><input type="text" name="name"></td></tr>
		<tr height=8%><td width=24%>
		&nbsp;&nbsp;&nbsp;&nbsp;IP Address</td><td> :  </td><td width=24%><input type="text" name="ip"></td></tr>
		<tr height=8%><td width=24%>
		...
		<input type="submit" name="submit" value="ADD INTERFACE"></th></tr>
		</table>
		</form>
		<?php
			...
			if($_POST['ip']==""&&$_POST['name']!="")
			echo "<b><i>SORRY !!!! IP ADDRESS FIELD CAN NOT BE EMPTY!!!</i></b>";
			if($_POST['network']==""&&$_POST['ip']!=""&&$_POST['name']!="")
			echo "<b><i>SORRY !!!! NETWORK ADDRESS FIELD CAN NOT BE EMPTY!!!</i></b>";	
			if($_POST['netmask']==""&&$_POST['network']!=""&&$_POST['ip']!=""&&$_POST['name']!="")
			echo "<b><i>SORRY !!!! NETWORK MASK FIELD CAN NOT BE EMPTY!!!</i></b>";
			if($_POST['broadcast']==""&&$_POST['netmask']!=""&&$_POST['network']!=""&&$_POST['ip']!=""&&$_POST['name']!="")
			echo "<b><i>SORRY !!!! BROADCAST ADDRESS FIELD CAN NOT BE EMPTY!!!</i></b>";
			}

		if(isset($_POST['submit'])&&$_POST['broadcast']!=""&&$_POST['netmask']!=""&&$_POST['network']!=""&&$_POST['ip']!=""&&$_POST['name']!="")
	                      {
				$a=$_POST['name'];
				$b=$_POST['ip'];
				$c=$_POST['network'];
				...
				if ($f!="")
				{	 
			             echo "<b>NETWORK INTERFACE ALREADY EXISTS!!!!";
				}
		               else
	                        {
				$v=explode(".",$b);
...
IPADDR=$b
NETWORK=$c
ONBOOT=yes' >> /etc/sysconfig/network-scripts/ifcfg-$a" > add.sh`;
				$g=`sudo bash add.sh`;
				$g=`sudo rm -rf add.sh`;
				echo "<b>NETWORK INTERFACE&nbsp;&nbsp;<u><b><i>$a</i></u>&nbsp;&nbsp;ADDED !!!!</b>";
				}
			      }
			     }				
			    }
