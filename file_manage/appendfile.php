<html>
	<head>
		<title>Append Existing File</title>
	</head>
	<body>
		...
		<h1><b><u> APPEND EXISTING FILE</u></b></h1>
		<table border=2><tr><th><img src=append.gif></th><tr></table><hr>
		<form action="" method="post">
		<table height=33% width=50% bgcolor=#cccccc>
		<tr bgcolor=#9999ff height=4%><td>
		<b>&nbsp;&nbsp;&nbsp;&nbsp;Append existing file</b></td></tr>
		<tr height=8%><th>
		File Name : <input type="text" name="folder"></th></tr>
		<tr height=8%><th>
		File Path (Absolute):  <input type="text" name="path"></th></tr>
		<tr height=8%><th>
		Content of file : <input type="text" name="content"></th></tr>
		<tr height=5%><th>
		<input type="submit" name="submit" value="Write file"></th></tr>
		</table>
		</form>
		<?php
		...
		if(isset($_POST['submit'])&&$_POST['folder']!=""&&$_POST['path']!="")
			{
				...
				if($d=="")
				{
					echo "<b><i>SORRY !!!! FILE NAME \"$a\" DOES NOT EXISTS IN PATH \"$b\" !!!!</b></i>";
				}
			   else
				{
				$l=`sudo touch a.sh`;
				$l=`sudo chmod 777 a.sh`;
				$l=`echo "echo $k >> $b/$a"> a.sh`;
				$l=`sudo bash a.sh`;
				$l=`sudo rm -rf a.sh`;
				echo "<b><i>FILE \"$a\" APPENDED IN \"$b\" !!!!</i></b>";
				}
			      }	
			   }
		?>
